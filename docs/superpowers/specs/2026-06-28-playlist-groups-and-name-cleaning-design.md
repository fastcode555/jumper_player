...
输入：`rawFileName`、`NameCleanConfig`。输出：`CleanedName { displayName, seriesTitle, episodeNumber, season }`。
...
6. **得 seriesTitle**：在 4/5 之后，从结果中移除集号 token（如 `第N集`、`EP12`、`SxxEyy`、
   行尾孤立数字）得到剧名；`displayName` = seriesTitle + 规范化集号（如 `剧名 01`）。
7. **兜底**：若清洗后为空，回退使用原始文件名（去扩展名）。
...
2. 对每个文件用 `NameCleaner`（注入当前配置）得到 `displayName / seriesTitle / 集号`。
